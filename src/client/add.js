...
const { getFolders } = require("../gallery");
...
ipcRenderer.on("savesFolder", (event, data) => {
    ...
    getFolders(musicFolder).then((deFolders) => {
        folders = deFolders;
        folderTo.innerHTML = deFolders
            .map((folder) => `<option>${folder}</option>`)
            .join("\n");
        folderTo.selectedIndex = deFolders.indexOf(
            path.parse(musicFolder).base
        );
    });
});
...
async function download(url) {
    if (!fs.existsSync(musicFolder))
        return addStatus(
            "please choose a folder then restart this window to be able to download songs"
        );

    if (
        url.match(
            /^https?:\/\/(www.youtube.com|music.youtube.com|youtube.com)\/playlist(.*)$/
        )
    ) {
        ...
        for (const video of playlist.videos) {
            try {
                ...
                report.innerText = `Done: ${downloaded}, Error: ${errored}, Total: ${
                    downloaded + errored
                }, of: ${toDownload}`;
                await downloadAudio(
                    {
        ...
        for (let i = 0; i < videos.length; i++) {
            ...
            btn.id = `${i}download`;
            btn.className =
                "px-3 py-[0.7] text-sm text-blue-600 font-semibold rounded-full border border-blue-200 hover:text-white hover:bg-blue-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2";
...
async function downloadAudio(
    { url, title, thumbnail, artist, originalTitle, album },
    playlist = false,
    plname = null
) {
    let thepath = "";
    ...
    if (pathConfig === path.parse(musicFolder).base) {
        ...
        thepath = playlist
            ? path.join(musicFolder, plname)
            : path.join(musicFolder, pathConfig);
    }
    ...
    videoReadableStream
        ...
        .on("error", (err) => {
            addStatus(
                `An error happened while downloading: ${title}\n${err.name}: ${err.message}`
            );
            errored++;
            report.innerText = `Done: ${downloaded}, Error: ${errored}, Total: ${
                downloaded + errored
        ...
        .on("end", async () => {
            if (!ffmpegFound) {
                ...
                try {
                    ...
                    if (settings["search"].status)
                        searchLyrics(
                            originalTitle,
                            fs.readFileSync(
                                path.join(savesPath, "lyrics.txt"),
                                "utf-8"
                            )
                        );
                }
                ...
                try {
                    ...
                    const metadata = `-metadata title="${changeName(
                        originalTitle
                    )}" -metadata artist="${artist}"`;

                    execSync(
                        `ffmpeg -i "${originalPath}" ${metadata} -b:a ${bitrate} "${outPath}"`
                    );
